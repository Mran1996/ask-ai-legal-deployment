          ...
          {activeTab === 'settings' ? (
            <div className="bg-white border rounded-xl shadow-sm p-4 md:p-6 max-w-xl w-full mx-auto">
              ...
              <div className="text-gray-700 mb-2 text-sm md:text-base">
                <div>Full Name: <span className="font-medium">{displayName}</span></div>
                <div>Email: <span className="font-medium">{email}</span></div>
                <div>Account Created: <span className="font-medium">{createdAt}</span></div>
              </div>
                ...
                <div className="rounded-md border border-green-200 bg-green-50 p-3 md:p-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mb-4">
                  ...
                  <div>
                    <div className="text-sm md:text-base font-medium">
                      You're on the <span className="font-extrabold">{planName}</span> Plan
                    </div>
                    <div className="text-xs md:text-sm mt-1 font-semibold">Next billing: <span className="font-normal">{nextBilling}</span></div>
                  </div>
                </div>
                <ul className="space-y-2 mb-6">
                  {features.map((feature, i) => (
                    <li key={i} className="flex items-start text-sm md:text-base font-medium"><span className="mr-2 mt-0.5">✅</span>{feature}</li>
                  ))}
                </ul>
                <div className="flex flex-wrap gap-3 mt-6">
                  <button className="flex items-center justify-center gap-2 border rounded-lg px-3 md:px-4 py-2 text-sm md:text-base font-bold text-black bg-white hover:bg-gray-100 hover:ring-1 hover:ring-gray-300 transition w-full sm:w-auto">
